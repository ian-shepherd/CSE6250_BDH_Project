...
def extract_features(df, links):
    ...
    c.compare_vectorized(
        reset_day, ("day", "month"), ("day", "month"), label="reset_day_flag"
    )
    c.compare_vectorized(
        swap_fields_flag, ("day", "month"), ("day", "month"), label="swap_day_month"
    )
    c.compare_vectorized(
        swap_fields_flag,
        ("surname", "given_name"),
...
def train_model(modeltype, modelparam, train_vectors, train_labels, modeltype_2):
    if modeltype == "svm":  # Support Vector Machine
        ...
        model = RandomForestClassifier(
            n_estimators=100,
            criterion=modeltype_2,
            max_depth=7,
            min_samples_leaf=1,
            min_weight_fraction_leaf=0.0,
            max_features="sqrt",
            max_leaf_nodes=None,
            min_impurity_decrease=modelparam,
            bootstrap=True,
            oob_score=False,
            random_state=0,
            warm_start=False,
...
def block_test_sets(df_test):
    ...
    for field in blocking_fields:
        ...
        print(
            "Number of pairs of matched " + field + ": " + str(len(candidates)),
            ", detected ",
            detects,
            "/"
            + str(leng_test_true_links)
            + " true matched pairs, missed "
            + str(leng_test_true_links - detects),
        )

    detects = blocking_performance(all_candidate_pairs, test_true_links, df_test)
    print(
        "Number of pairs of at least 1 field matched: " + str(len(all_candidate_pairs)),
        ", detected ",
        detects,
        "/"
        + str(leng_test_true_links)
...
def generate_test_sets(df_test, all_candidate_pairs):
    ...
    for i in range(0, len(feature_index)):
        if (
            df_test.loc[feature_index[i][0]]["match_id"]
            == df_test.loc[feature_index[i][1]]["match_id"]
        ):
            labels[i] = 1
...
def train_test_models(X_train, y_train, X_test, y_test):
    ...
    modelparam_range = [
        ...
        2000,
        5000,
    ]  # C for svm, C for lg, alpha for NN
...
def bagging_model(X_train, y_train, X_test, y_test):
    ...
    modeltypes_2 = ["rbf", "relu", "l2", "entropy"]
    modelparams = [0.001, 2000, 0.005, 0.2]
    ...
    for model_i in range(len(modeltypes)):
        ...
        for train_index, valid_index in kf.split(X_train):
            ...
            md = train_model(
                modeltype, modelparam, X_train_fold, y_train_fold, modeltype_2
            )
        ...
        model = (
            modeltypes[model_i]
            + " - "
            + modeltypes_2[model_i]
            + " - "
            + str(modelparams[model_i])
        )
...
def main(local=False):
    ...
    df_precision, df_sensitivity, df_fscore, df_nb_false = train_test_models(
        X_train, y_train, X_test, y_test
    )

    pl.show_plot(
        df_precision,
        "Hyperparameter - Precision - Scheme B",
        "precision",
        "precision_schemeB_1.png",
    )
    pl.show_plot(
        df_sensitivity,
        "Hyperparameter - Sensitivity - Scheme B",
        "sensitivity",
        "sensitivity_schemeB_1.png",
    )
    pl.show_plot(
        df_fscore,
        "Hyperparameter - FScore - Scheme B",
        "fscore",
        "fscore_schemeB_1.png",
    )
    pl.show_plot(
        df_nb_false,
        "Hyperparameter - Falses - Scheme B",
        "#falses",
        "falses_schemeB_1.png",
    )
