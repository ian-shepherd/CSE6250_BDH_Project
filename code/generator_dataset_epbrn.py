...
def process_record(rc_input, df, all_error_types, all_error_weights, all_fields):
    ...
    errortypes = np.random.choice(all_error_types, no_error, p=all_error_weights)
    rc = rc_input.copy()
    for errortype in errortypes:
        ...
        if errortype == "ph1":
            if len(str(rc["phone_number"])) != "00-00000000":
                phone = df.iloc[random.choice(range(len(df)))]["phone_number"]
                rc["phone_number"] = phone[0:2] + rc["phone_number"][2:11]
        if errortype == "ph2":
            if len(str(rc["phone_number"])) != "00-00000000":
                change_digits = random.choice(range(3)) + 1
                phone = list(str(rc["phone_number"]))
                new_phone = phone.copy()

                for _ in range(change_digits):
                    change_pos = random.choice(range(3, 9))
                    new_phone[change_pos] = phone[change_pos + 1]
                    new_phone[change_pos + 1] = phone[change_pos]
                rc["phone_number"] = "".join(new_phone)
    return rc
...
def preprocess(inputfile):
    ...
    df["phone_number"] = (
        df["phone_number"].replace("\s+", "-", regex=True).fillna("00-00000000")
    )

    df = df.drop(["age", "soc_sec_id", "blocking_number", "date_of_birth"], axis=1)
...
def generate_links(df, inputfile, count_shared):
    ...
    print(
        "Total records after generated:",
        leng + no_double_linked + no_triple_linked * 2 + no_quad_linked * 3,
    )
    print(
        "Matched pairs:", no_double_linked + no_triple_linked * 3 + no_quad_linked * 6
    )
...
def generate_files(PATH_FILES, type_set):
    ...
    ph1 = 1  # change area code on phone number
    ph2 = 1  # swap random digits on phone number
    ...
    list_double_linked, list_triple_linked, list_quad_linked = generate_links(
        df, inputpath, count_shared
    )
